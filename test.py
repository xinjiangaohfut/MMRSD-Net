...
def main():
    ...
    Remmodel.load_state_dict(torch.load('./pretrain/Remderain-660.pkl'))#############################################
    Hfmodel.load_state_dict(torch.load('./pretrain/Hf-575.pkl'))


    y = cv2.imread('./sample/realworld001.png')
    ...
    with torch.no_grad():
        ...
        save_image(Rem_Output, './result/derain.png')
        ...
        plt.margins(0,0)
        plt.savefig('./result/rainstreaks.png',dpi=300)############################################
        plt.close()
